...
This package deploys the PBORA Form 13 Flask app to the Ubuntu/PostgreSQL host at `10.107.20.200` and serves the LAN app at:

```text
http://10.107.20.200:8000
```

Current deployment values:

- Ubuntu app server IP: `10.107.20.200`
...
```bash
sudo sed -i "s/^#listen_addresses =.*/listen_addresses = 'localhost,10.107.20.200'/" /etc/postgresql/*/main/postgresql.conf
...
```bash
PGPASSWORD=pbora psql -h 10.107.20.200 -p 5432 -U pbora -d pbora -c "select current_database(), current_user;"
```
...
```text
PUBLIC_BASE_URL=http://10.107.20.200:8000
ALLOWED_HOSTS=10.107.20.200,localhost,127.0.0.1,returnsform14.onrender.com,www.returnsform14.org,returnsform14.org
INTERNAL_DATABASE_URL=postgresql+psycopg2://pbora:pbora@10.107.20.200:5432/pbora
DB_HOST=10.107.20.200
DB_NAME=pbora
DB_USER=pbora
DB_PASSWORD=pbora
DB_SSL_MODE=disable
...
Keep PostgreSQL on the same LAN host and keep `DB_SSL_MODE=disable` for the private `10.107.20.200` address.
