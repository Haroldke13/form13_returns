...
APP_HOST_IP="$(read_env_value PBORA_APP_HOST_IP "$(server_ip)")"
...
if [[ -z "$LAN_CIDR" ]]; then
  CIDR_SOURCE_IP="$APP_HOST_IP"
  if is_auto_env_value "$CIDR_SOURCE_IP"; then
    CIDR_SOURCE_IP="$(server_ip)"
  fi
  if [[ ! "$CIDR_SOURCE_IP" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    CIDR_SOURCE_IP="$DB_HOST"
  fi
  if [[ ! "$CIDR_SOURCE_IP" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    die "Could not infer POSTGRES_ALLOWED_CIDR. Set POSTGRES_ALLOWED_CIDR in $ENV_FILE."
  fi
  IFS=. read -r ip1 ip2 ip3 _ <<<"$CIDR_SOURCE_IP"
